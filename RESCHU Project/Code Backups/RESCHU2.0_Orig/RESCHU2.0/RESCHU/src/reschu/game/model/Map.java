...
public class Map {
	...
	public LinkedList<Target> getListAssignedTarget() {return listAssignedTarget; }
	public LinkedList<Target> getListUnassignedTarget() {return listUnassignedTarget; }
	public LinkedList<int[]> getListHazard() {return listHazard; }
	
	public int getTargetSize(String target_type) {
		int cnt = 0;
		for(int i = 0; i < listAssignedTarget.size(); i++) 
			if( listAssignedTarget.get(i).getMission() == target_type ) cnt++;
		for(int i = 0; i < listUnassignedTarget.size(); i++) 
			if( listUnassignedTarget.get(i).getMission() == target_type ) cnt++;	
		return cnt;
	}	
	
	public synchronized void setCellType(int x, int y, int type) { map_array[x][y] = type; }

	public synchronized int getCellType(int x, int y) { return map_array[x][y]; }
	
	public synchronized void addTarget(Target t) {
		if (g.isRunning() ) lsnr.EVT_Target_Generated(t.getName(), t.getPos(), t.isVisible());
		listUnassignedTarget.addLast(t); 
	}

	public synchronized void addHazard(int x, int y, int size) {		
		if( g.isRunning() ) lsnr.EVT_HazardArea_Generated(new int[]{x, y});
	...
	public synchronized boolean isTarget(int x, int y) {
		if( isAssignedTarget(x, y) || isUnassignedTarget(x, y)) return true;
		return false;
	}
    ...
    private boolean boundaryCheck(int x, int y, int[] target_pos) {
    	...
    	for( int i=-w; i<w; i++)
    		for( int j=-w; j<w; j++)
    			if( x+i==target_pos[0] && y+j==target_pos[1] ) return true; 
    	return false;
    }
    ...
    private boolean chkOkayToAdd(int x, int y) {
    	...
    	for(int i=0; i<listAssignedTarget.size(); i++) {
    		...
    		if(Game.getDistance(t.getPos()[0], t.getPos()[1], x, y ) < LIMIT_DISTANCE) {
    			...
    			if(DEBUG) System.err.println("LAT ("+x1+","+y1+") with randomly generated ("+x2+","+y2+"), d=("+d+")");
    			return false;
    		}
    	}
    	for(int i=0; i<listUnassignedTarget.size(); i++) {
    		...
    		if(Game.getDistance(t.getPos()[0], t.getPos()[1], x, y ) < LIMIT_DISTANCE) {
    			int x1 = t.getPos()[0], y1 = t.getPos()[1], x2 = x, y2 = y;
    			double d = Game.getDistance(t.getPos()[0], t.getPos()[1], x, y );
    			if(DEBUG) System.err.println("LUT("+x1+","+y1+") with randomly generated ("+x2+","+y2+"), d=("+d+")");
    			return false;
    		}
    	}
    	for( int i=0; i<listHazard.size(); i++ ) {
    		if( Game.getDistance(x, y, listHazard.get(i)[0], listHazard.get(i)[1]) < LIMIT_DISTANCE ) {
    			...
    			if(DEBUG) System.err.println("LHA("+x1+","+y1+") with randomly generated ("+x2+","+y2+"), d=("+d+")");
    			return false; 
    		}
    	} 
    ...
    public void setTargetArea(Random rnd) throws UserDefinedException { 
        ...
        for(int i=0; i<nLandTargetNeed; i++) {
        	int x, y;
        	do { 
        		x = rnd.nextInt(MySize.width); y = rnd.nextInt(MySize.height);
        		if( ++cnt >= limit ) {
        			throw new UserDefinedException("setTargetArea(land) try limit exceed");
        		}
        	} while( !(getCellType(x, y)!=MyGame.SEA && chkOkayToAdd(x, y)) );
        	Target t = new Target(g.getEmptyTargetName(), chkTargetOffset(x, y), "LAND", "UAV", g.getTargetVisibility());
        	addTarget(t);
        } 
        for(int i=0; i<nShoreTargetNeed; i++) {
        	...
        	} while( !(getCellType(x, y)==MyGame.SEASHORE && chkOkayToAdd(x, y)) ); // @change yale 2008-06-29 || !chkOkayToAdd(x,y) ); 
        	Target t = new Target(g.getEmptyTargetName(), chkTargetOffset(x, y), "SHORE", "UUV", g.getTargetVisibility());
        	addTarget(t);
        }
        for(int i=0; i<nCommTargetNeed; i++) {
        	...
        	Target t = new Target(g.getEmptyTargetName(), chkTargetOffset(x, y), "COMM", "UAV", g.getTargetVisibility());
        	addTarget(t);
        }
    } 
    ...
    private int[] chkTargetOffset(int x, int y) {
    	...
    	if( x > MySize.width-offset ) x -= offset;
    	if( y > MySize.height-offset ) y-= offset;
    	return new int[]{x, y};
    }
    ...
    public int getAvailableTarget() {
    	...
    	for(int i=0; i<getListUnassignedTarget().size(); i++ ) {
    		if( getListUnassignedTarget().get(i).isVisible() ) cnt++;
    	}
    	return cnt;
    }
    ...
    //    public void manual_setTargetArea(int x, int y, String Mission_Type, String Vehicle_Type) {
    //       Target t = new Target(g.getEmptyTargetName(), chkTargetOffset(x, y), Mission_Type, Vehicle_Type, g.getTargetVisibility());
    //       addTarget(t);
    //    }    
    ...
    private boolean chkOkayToAdd_TEMPORARY_FOR_TUTORIAL_BY_CARL(int x, int y) {
    	for(int j=0; j<listAssignedTarget.size(); j++) {
    		...
    		if(Game.getDistance(t.getPos()[0], t.getPos()[1], x, y ) < MySize.SIZE_HAZARD_3_PXL)
    			//System.err.println("Target[" + t.getName() +"] d=" + Game.getDistance(t.getPos()[0], t.getPos()[1], x, y ));
    			return false;
    	}
    	for(int i=0; i<listUnassignedTarget.size(); i++) {
    		Target t = listUnassignedTarget.get(i);
    ...
    public void setTargetArea_TEMPORARY_FOR_TUTORIAL_BY_CARL(Random rnd) { 
        ...
        for(int i=0; i<nShoreTargetNeed; i++) {
        	...
        	do {
        		...
        		x = rnd.nextInt(MySize.width); y = rnd.nextInt(MySize.height); 
                        //if (getCellType(y,x)!=MyGame.SEASHORE) System.out.println("LAND TYPE PROB");
